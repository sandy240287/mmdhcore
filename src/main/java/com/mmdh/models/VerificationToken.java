...
public class VerificationToken extends AuditEntity implements Serializable {
	...
	public VerificationToken(String token, MMDHUser user) {
		super();
		this.setToken(token);
		this.user = user;
	...
	@OneToOne(targetEntity = MMDHUser.class, fetch = FetchType.EAGER, cascade = CascadeType.REMOVE)
	...
	public void setUser(MMDHUser user) {
		this.user = user;
	}
